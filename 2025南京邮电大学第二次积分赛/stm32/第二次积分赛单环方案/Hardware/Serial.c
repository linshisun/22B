...
char Serial2_RxPacket[100];	//定义接收数据包数组，数据包格式"@MSG\r\n"
...
{
	/*开启时钟*/
	RCC_APB1PeriphClockCmd(RCC_APB1Periph_USART2, ENABLE);	//开启USART3的时钟,注意函数名！
	...
	GPIO_Init(GPIOA, &GPIO_InitStructure);					//将PB10引脚初始化为复用推挽输出
	...
	USART_Init(USART2, &USART_InitStructure);				//将结构体变量交给USART_Init，配置USART3
	...
	NVIC_InitStructure.NVIC_IRQChannel = USART2_IRQn;		//选择配置NVIC的USART3线
	...
	/*USART使能*/
	USART_Cmd(USART2, ENABLE);								//使能USART3，串口开始运行
}
...
void USART2_IRQHandler(void)
{
	...
	if (USART_GetITStatus(USART2, USART_IT_RXNE) == SET)	//判断是否是USART3的接收事件触发的中断
	{
		...
		if (RxState == 0)
		{
			if (RxData == '@' && Serial2_RxFlag == 0)		//如果数据确实是包头，并且上一个数据包已处理完毕
			{
				RxState = 1;			//置下一个状态
